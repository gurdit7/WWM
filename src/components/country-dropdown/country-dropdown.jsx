...
import useCurrency from "@/contexts/currency-provider/currency-provider";
  ...
  return (
    <Wrapper className="relative">
      <button
        onClick={() => setOpen((s) => !s)}
        type="button"
        className={` ${className || ''} w-[100.99px] h-[45px] border text-[13px] leading-[19.5px] rounded-[6px] font-semibold justify-center px-4 flex gap-x-4 items-center border-[#DCDAD8]`}
      >
        {countries.map((country, i) => (
          <>
            {country.code && country.code === selectedCountry && (
              <>
                <Image
      ...
      <Wrapper
        ...
        } transtion duration-500  left-0 absolute overflow-y-auto bg-white ${footer ? "bottom-full" : "top-full"}`}
      >
        <ul>
          {countries.map((country, i) => (
            <>
              {country.code && (
                <li key={i}>
                  <button
                    type="button"
                    onClick={() => setCountry(country.code, country.currency_code)}
